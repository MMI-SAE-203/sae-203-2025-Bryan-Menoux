...
const $$Hero = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div${addAttribute((heroImg ? "h-full bg-transparent" : "bg-(--color-green) max-h-[45dvh] lg:max-h-[70dvh] items-start") + " hero relative h-screen text-white flex flex-col", "class")}> <div${addAttribute((heroImg ? "absolute h-screen inset-0 -z-10" : "relative h-0") + (hero.duotone ? " duotone" : ""), "class")}> ${heroImg && renderTemplate`<img loading="lazy"${addAttribute(heroImg, "src")} alt="Hero Image" class="w-full h-full block lg:hidden">`} ${heroImgLg && renderTemplate`<img loading="lazy"${addAttribute(heroImgLg, "src")} alt="Hero Image Large" class="w-full h-full hidden lg:block">`} </div> <div${addAttribute((heroImg ? "absolute inset-0 h-full w-full" : "relative mt-0 lg:mt-32 w-full") + " flex flex-col justify-center w-full gap-4 relative z-10 px-4 lg:px-16 pt-34", "class")}> <h1>${hero.title}</h1> <div> <p>${hero.description}</p> <p>${hero.date}</p> </div> ${hero.button && renderTemplate`${renderComponent($$result, "Button", $$Button, {}, { "default": ($$result2) => renderTemplate` <a href="/programme/">${hero.button}</a> ` })}`} </div> <div class="absolute bottom-0 left-0 right-0 flex justify-center py-8 z-10"> <a href="#first"> <img${addAttribute(fleche.src, "src")} alt="flèche vers le bas"> </a> </div> </div> ${renderScript($$result, "C:/Users/Nekros/Documents/GitHub/R213_agence/sae-203-2025-Bryan-Menoux/src/components/Hero.astro?astro&type=script&index=0&lang.ts")}`;
